...
export async function loadMvpSeedFromPostgres(connectionString?: string): Promise<MvpSeed> {
  ...
  try {
    const [
      ...
      db.select().from(levelScales),
      db.select().from(levelDimensionDescriptors),
    ...
    return {
      ...
      levels: levelRows.map((level) => ({
        value: level.value,
        scaleId: level.scaleId ?? undefined,
        title: level.title,
        description: level.description,
      })),
      levelScales: levelScaleRows.map((scale) => ({
        id: scale.id,
        organizationId: scale.organizationId,
        name: scale.name,
        isDefault: scale.isDefault,
        status: scale.status as 'draft' | 'active' | 'archived',
      })),
      levelDimensionDescriptors: levelDimensionDescriptorRows.map((descriptor) => ({
        id: descriptor.id,
        scaleId: descriptor.scaleId,
        levelValue: descriptor.levelValue,
        dimension: descriptor.dimension as 'autonomy' | 'complexity' | 'influence' | 'support' | 'impact',
        description: descriptor.description,
      })),
      scoringRules: scoringRuleRows.map((rule) => ({
        id: rule.id,
