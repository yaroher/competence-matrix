...
@Component({
  ...
  template: `
    <section class="section">
      <header class="section-head"><div><span class="eyebrow">Administration</span><h2>People & organization</h2></div></header>
      <div class="grid3">
        <article class="panel">
          <h3>People</h3>
          <div class="form-row"><input class="fld grow" [(ngModel)]="personName" placeholder="Full name"/><input class="fld grow" [(ngModel)]="personEmail" placeholder="email"/><button z-button zType="secondary" zSize="sm" (click)="addPerson()" [disabled]="!personName().trim()||!personEmail().trim()">Add</button></div>
        ...
        <article class="panel">
          <h3>Org units</h3>
          <div class="form-row">
            <select class="fld" [(ngModel)]="unitParent"><option [ngValue]="null">— root —</option>@for (u of orgUnits(); track u.id) { <option [value]="u.id">{{ u.name }}</option> }</select>
            ...
            <input class="fld grow" [(ngModel)]="unitName" placeholder="unit name"/>
            <button z-button zType="secondary" zSize="sm" (click)="addUnit()" [disabled]="!unitName().trim()">Add</button>
          </div>
        ...
        <article class="panel">
          ...
          <div class="form-row">
            ...
            <button z-button zType="secondary" zSize="sm" (click)="addAsg()" [disabled]="!asgPerson()||!asgUnit()">Assign</button>
          </div>
