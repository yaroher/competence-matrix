...
@Component({
  ...
  template: `
    <section class="section">
      <header class="section-head"><div><span class="eyebrow">Growth</span><h2>Development plans</h2></div></header>
      <article class="panel">
        <div class="form-row">
          <label class="muted">Assessment</label>
      ...
      @if (plan(); as p) {
        <article class="panel">
          ...
          @if (p.items.length) {
            <table class="crud"><thead><tr><th>Item</th><th>Gap</th><th>Status</th><th>Due</th><th></th></tr></thead><tbody>
          ...
          } @else { <p class="muted">No development items.</p> }
        </article>
      } @else { <p class="muted">Select an assessment.</p> }
    </section>
  `,
